...
- (int)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
@end
...
- (int) executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    //TODO id
    sqlite3_prepare_v2(_db,"select last_insert_rowid()",-1,&pStmt,NULL);
    int _id = 0;
    while (sqlite3_step(pStmt)==SQLITE_ROW){
        _id = sqlite3_column_int(pStmt, 0);
    }
    ...
    _isExecutingStatement = NO;
    return (rc == SQLITE_DONE || rc == SQLITE_OK)?_id:0;
}
...
- (int)executeUpdate:(NSString*)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeUpdate:sql error:nil withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (int)executeUpdate:(NSString*)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (int)executeUpdate:(NSString*)sql withVAList:(va_list)args {
