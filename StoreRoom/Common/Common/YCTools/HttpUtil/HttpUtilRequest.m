    ...
    for (NSString *key in [dicParam allKeys]) {
        ...
        if (value) {
            if ([value isKindOfClass:[NSString class]]) {
                ...
                stringParams = [NSString stringWithFormat:@"%@&%@=%@",stringParams,key,[((NSNumber*)value) stringValueWithPrecision:8]];
                ...
                stringParams = [NSString stringWithFormat:@"%@&%@=%@",stringParams,key,[value JSONRepresentation]];
            }
        }
    }
...
-(NSMutableURLRequest*) createUrlRequest:(NSDictionary*) params OutTime:(int) outTime{
    NSString *temp;
    if ([[self.requestString componentsSeparatedByString:@"?"] count]==2) {
        temp = params?[NSString stringWithFormat:@"%@&%@",self.requestString,[[self checkParamsConstructionToNormarl:params] stringByAddingPercentEscapesUsingEncoding:self.encoding]]:self.requestString;
    }else{
        temp = params?[NSString stringWithFormat:@"%@?%@",self.requestString,[[self checkParamsConstructionToNormarl:params] stringByAddingPercentEscapesUsingEncoding:self.encoding]]:self.requestString;
    }
    
    printf("\nhttpurl:%s",[temp UTF8String]);
...
-(void) startAsynRequest{
    ...
    [_connection start];
    printf("\nhttpurl:%s",[_requestString UTF8String]);
}
