...
//        NSHashTable *map = [NSHashTable weakObjectsHashTable];
        NSValue *value = [NSValue valueWithPointer:(__bridge const void *)(target)];
        if (arguments) {
            [self.dic setObject:@{@"target":value,@"selstr":selstr,@"arguments":arguments} forKey:key];
        }else{
            [self.dic setObject:@{@"target":value,@"selstr":selstr} forKey:key];
        }
        return true;
    }
    return false;
...
//        NSMapTable *map = [dicobj objectForKey:@"target"];
        NSValue *value = [dicobj objectForKey:@"target"];
        id target = value.pointerValue;//[map objectForKey:@"target"];
