...
@property (nonatomic) float titleOffy;
@property (nonatomic) float messageOffy;
...
+(void) initialize{
    ...
    STATIC_DIALOG_BUTTON_HEIGHT=44.0f;
    
    STATIC_DIALOG_BUTTON_NAME_FONT = [UIFont systemFontOfSize:[Utils getFontSizeWithHeight:STATIC_DIALOG_BUTTON_HEIGHT*0.7 fontName:tempfont.fontName]];
    ...
    STATIC_DIALOG_BORDER_COLOR = [UIColor colorWithRed:0 green:0 blue:0 alpha:0];
...
-(void) setAllView{
    ...
    UIView *viewShow = [self createViewShow];
...
-(UIView*) createButtonWithNames:(NSArray*) names viewWidth:(float) viewWidth{
    ...
    if([names count]==2){
        ...
        r.origin.x = viewWidth-r.size.width+1;
        r.size.width -=1;
        ...
        for (NSString *name in names) {
            ...
            offy = r.origin.y + button.frame.size.height;
        }
    }
