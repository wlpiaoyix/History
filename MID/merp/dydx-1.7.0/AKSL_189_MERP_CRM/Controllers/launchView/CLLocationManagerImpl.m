...
-(void) forStopUpdating{
    ...
    @try {
        ...
        while (!self.stopUpdateFlag) {
            ...
            NSLog(@"%d======%d",[NSThread currentThread].hash,index);
            if (index>30) {
                break;
            }
            index++;
        }
        NSLog(@"%d====== stopUpdatingLocation",[NSThread currentThread].hash);
        [self.manager stopUpdatingLocation];
    }
    @finally {
        [self.lock unlock];
        NSLog(@"%d====== unlock",[NSThread currentThread].hash);
    }
}
/*
