...
- (void)setUp
{
    ...
    _pageIndicatorViewSize = CGSizeMake(10., 5.);
}
...
-(void)setPageIndicatorVIewFrame:(NSUInteger)selectedIndex{
    CGRect frametemp = self.pageIndicatorView.frame;
    frametemp.size.width = [self.topBar widthForSelectedItemAtIndex:selectedIndex];
    self.pageIndicatorView.frame = frametemp;
    //[self.pageIndicatorView setNeedsDisplay];
}
#pragma mark - Public
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated
{
    ...
    [self setPageIndicatorVIewFrame:selectedIndex];
    ...
    if (abs(self.selectedIndex - selectedIndex) <= 1) {
        ...
        [UIView animateWithDuration:(animated) ? 0.3 : 0. delay:0. options:UIViewAnimationOptionBeginFromCurrentState animations:^
         {
             ...
             [nextSelectdItem setTitleColor:[UIColor colorWithRed:0.306 green:0.561 blue:0.110 alpha:1] forState:UIControlStateNormal];
         } completion:nil];
    } else {
        ...
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            [previosSelectdItem setTitleColor:[UIColor colorWithWhite:0.6 alpha:1.] forState:UIControlStateNormal];
            [nextSelectdItem setTitleColor:[UIColor colorWithRed:0.506 green:0.706 blue:0.063 alpha:1] forState:UIControlStateNormal];
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(DidSelectedIndex:)]) {
		[_delegate DidSelectedIndex:_selectedIndex];
	}
}
...
- (void)setViewControllers:(NSArray *)viewControllers
{
    if (_viewControllers != viewControllers) {
        ...
        self.pageIndicatorView.center = CGPointMake([self.topBar centerForSelectedItemAtIndex:self.selectedIndex].x,
                                                    self.pageIndicatorView.center.y);
        [self setPageIndicatorVIewFrame:self.selectedIndex];
    }
}
...
- (void)layoutSubviews
{
    ...
    self.pageIndicatorView.center = CGPointMake([self.topBar centerForSelectedItemAtIndex:self.selectedIndex].x,
                                                self.pageIndicatorView.center.y-self.pageIndicatorViewSize.height-1);
    [self setPageIndicatorVIewFrame:self.selectedIndex];
...
{
    CGFloat height = CGRectGetHeight(self.view.frame) - self.topBarHeight;
    return height;
}
...
//            [previosSelectedItem setTitleColor:[UIColor colorWithWhite:0.2 + 0.4 * (1 - fabsf(ratio))
//                                                                 alpha:1.] forState:UIControlStateNormal];
//            [nextSelectedItem setTitleColor:[UIColor colorWithWhite:0.2 + 0.4 * fabsf(ratio)
//                                                              alpha:1.] forState:UIControlStateNormal];
            if (scrollingTowards) {
