...
-(oneway void) release{
    NSLog(@"(release)show view in current thread hash is :%lu",(unsigned long)[[NSThread currentThread] hash]);
    [super release];
}
-(void) dealloc{
    NSLog(@"(delloc) show view in current thread has is :%lu",(unsigned long)[[NSThread currentThread] hash]);
    [super dealloc];
}
/*
