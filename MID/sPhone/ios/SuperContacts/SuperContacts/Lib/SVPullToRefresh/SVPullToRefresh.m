...
- (id)initWithScrollView:(UIScrollView *)scrollView {
    ...
    titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 6, 150, 20)];
    titleLabel.text = NSLocalizedString(@"下拉刷新...",);
    ...
    self.frame = CGRectMake(40, -35, scrollView.bounds.size.width, 35);
...
- (UIImageView *)arrow {
    if(!arrow) {
        ...
        arrow.frame = CGRectMake(0, 0, 15, 33);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最新更新: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"尚未更新",)];
}
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateTriggered:
            titleLabel.text = NSLocalizedString(@"释放刷新...",);
            [self rotateArrow:M_PI hide:NO];
            break;
            
        case SVPullToRefreshStateLoading:
            titleLabel.text = NSLocalizedString(@"加载数据...",);
