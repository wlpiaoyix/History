...
+(id) getSingleInstance{
    @synchronized(xrightController){
        if(!xrightController){
            ...
            xrightController.flcs = COMMON_FLCS;
...
-(void) queryData:(NSString*) params{
    ...
    _contantDatas = [_fdbs queryEntityUserByParams:params IfChekNum:YES];
    [((NSMutableArray*)_contantDatas) addObjectsFromArray:[_flcs queryByParams:params]];
    [_headHC removeAllObjects];
    [_dataArray removeAllObjects];
    int count = 0;
    int maxl = (int)[_contantDatas count];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSInteger count = [_dataArray count]?[((NSArray*)[_dataArray objectAtIndex:section]) count ]:0;
    return count;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(self.ifnothidden)[self.searchBarInfo resignFirstResponder];
    int _indexForTable = ((int)indexPath.section);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CTM_ViewContentController *c = [CTM_ViewContentController getNewInstance];
