...
@property (strong, nonatomic) SABFromLocationContentService *flcs;
...
+(id) getSingleInstance{
    @synchronized(xmainController){
        if(!xmainController){
            ...
            xmainController.flcs = COMMON_FLCS;
...
-(void) refreshRecordByParams:(NSString*) params{
    ...
    if([NSString isEnabled:params]){
        entityUserArray = (NSMutableArray*)[_flcs queryByParams:params];
    }
    @try {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([temp isKindOfClass:[NSMutableDictionary class]]){
        ...
        [crc setComing:r.statusCall.intValue];
        [crc setAction:count];
        [crc setRecord:r];
        [crc setComing:0];
        __weak typeof(self) tempself = self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
   ...
   _tableCurSelectephone = record.callPhoneNum;
    _index = 0;
    if([record getEntityUser]&&[record getEntityUser].userKey){
        _statusIndex = 0;
                ...
                case 0:
                {
                    ...
                    switch (buttonIndex) {
                        ...
                        case 2:
                        {
                            ...
                            EntityPhone *ep = [EntityPhone new];
                            ep.phoneNum = _tableCurSelectephone;
                            user.telephones = [NSMutableArray arrayWithObjects:ep, nil];
