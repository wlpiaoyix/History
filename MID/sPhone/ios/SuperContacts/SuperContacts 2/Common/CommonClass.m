...
static EntityManagerOperation *COMMONEM;//数据库操作对象
...
+(void) initialize{
    ...
    COMMONEM = [[EntityManagerOperation alloc]initDataBaseName:COMMON_DATABASE_NAME];
    [COMMONEM checkEntityCreate];
...
+(void)showMessage:(NSString*) message Title:(NSString*) title Delegate:(id<UIAlertViewDelegate>) delegate{
    [[[UIAlertView alloc] initWithTitle:title  message:message delegate:delegate cancelButtonTitle:@"确定" otherButtonTitles:nil]show];
}
+(void)showMessage:(NSString*) title Delegate:(id<UIAlertViewDelegate>) delegate Messages:(NSString*) message,...{
    va_list arg_ptr = NULL;
    va_start(arg_ptr, message);
    @try {
        UIAlertView *tempx = [[UIAlertView alloc] initWithTitle:title  message:nil delegate:delegate cancelButtonTitle:message otherButtonTitles:va_arg(arg_ptr, NSString*),va_arg(arg_ptr, NSString*),va_arg(arg_ptr, NSString*),va_arg(arg_ptr, NSString*),va_arg(arg_ptr, NSString*),nil];
        [tempx show];
    }
    @finally {
        va_end(arg_ptr);
...
+(void) refreshAllAddressBookRecords{
    ...
    COMMONADDRESSBOOKARRAY = [em queryAllRecord];
}
...
+(NSArray*) checkShortPingYing{
    @synchronized(COMMONUSERSHORTPY){
        if(!COMMONUSERSHORTPY){
            EntityManagerOperation *em = [CommonClass getCurrentEntityManager];
            COMMONUSERSHORTPY = [em excueSpecialTarget:^NSString *(id target) {
                return @"select u.shortPingYing from EntityUser as u";
            } ExcuCallBackBindValue:nil ExcuCallBackSetValue:^id(sqlite3_stmt *statement) {
                NSMutableArray *temp1 = [[NSMutableArray alloc] init];
                while (sqlite3_step(statement)==SQLITE_ROW){
                    const unsigned char *shortPingYing=sqlite3_column_text(statement,0);
                    [temp1 addObject:[NSString stringWithFormat:@"%s",shortPingYing]];
                }
                return temp1;
            }];
        }
...
+(NSMutableArray*) checkNumToPingYingForSql: (NSArray*) pys dicArg:(NSString*) dicArg SQLArray:(NSMutableArray*) sqlArray Index:(int) index{
    ...
    for(NSString *temp2 in temp1){
        ...
        if([pys count]>index+1){
            [self checkNumToPingYingForSql:pys dicArg:dicArg2 SQLArray:sqlArray Index:++index];
            --index;
        }else{
