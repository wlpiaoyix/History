...
+(bool) setConfigCache:(NSString *)key Value:(id)value{
    ...
    NSString *currentAccount = [ConfigManage getConfigTemp:COMMON_KEY_AT];
...
+(ModleUser*) getCurrentUser{
    @synchronized(currentUser){
        if(!currentUser){
            currentUser = [ConfigManage parseJSONtoClass:CMCK_ACCONTINFO];
        }
    }
    return currentUser;
}
...
+(void) setCacheUser:(ModleUser*) user{
    cacheUser = user;
    if(cacheUser)[ConfigManage setConfigTemp:[[user toJSON] JSONRepresentation] Value:@"tempUser"];
    else [ConfigManage setConfigTemp:false Value:@"tempUser"];
}
...
+(ModleUser*) parseJSONtoClass:(NSString*) key{
    ...
    if(json){
        ...
        user.key = [[json objectForKey:@"id"] longLongValue];
