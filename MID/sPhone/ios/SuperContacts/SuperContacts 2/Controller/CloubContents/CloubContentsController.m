...
- (IBAction)clickCopy:(id)sender {
    ...
    NSArray *array = [fdbs queryEntityUserByPhone:@""];
...
-(void) reloadTagInfo{
    ...
    if([NSString isEnabled:copyTime]){
        @try {
            copyTime = [[[NSDate alloc] initWithTimeIntervalSince1970:copyTime.floatValue] dateFormateDate:@"yyyy-MM-dd"];
        }
        @catch (NSException *exception) {
            copyTime = @"未知";
        }
...
- (void)connectionDidFinishLoading:(NSURLConnection*)theConnection {
    finished = YES;
    COMMON_SHOWMSGDELEGATE(@"确定要恢复当前通信录吗?", self, @"是",@"否");
    [self hideActivityIndicator];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    switch (buttonIndex) {
        case 0:
        {
            NSArray *users = [fdbs queryEntityUserByName:@""];
            for (EntityUser *user in users) {
                [MADE_COMMON deleteImage110ak640:user.dataImage];
                [fdbs removeEntityUser:user];
            }
